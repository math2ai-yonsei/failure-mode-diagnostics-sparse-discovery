...
class Gate3Config:
    ...
    # Track B parameters (D2)
    track_b_alpha: float = 0.3  # x_penalty weight
    track_b_min_dist_ratio: float = 0.1  # Diversity threshold
...
def track_b_selection(
    ...
    GPT P0-1~P0-6 반영:
    - Stage2-a: Score 계산 (theta excitation - x penalty)
    - Stage2-b: Greedy farthest-point selection
    - θ 주기 처리 (sin/cos)
    - Median/IQR 정규화
    - 아티팩트 저장
    - Canonicalize (global_idx 정렬)
    ...
    Args:
        ...
        min_distance_ratio: Diversity threshold
        rng: Random generator (for reproducibility, not used in deterministic selection)
        results_dir: Directory to save artifacts
        
    Returns:
        Dict with selected trajectories, artifacts, and metadata
    """
    print(f"\n[Track B Selection] n_select={n_select}, alpha={alpha}")
    
    trajectories = pool['trajectories']
    N_pool = len(trajectories)
    
    # 1. Compute features for ALL pool candidates
    features = compute_track_b_features(trajectories)  # (N_pool, 5)
    print(f"  Features computed: shape={features.shape}")
    
    # 2. Get Track A passed candidates
    ...
    # 3. Compute normalization stats from Track A passed (GPT P0-3)
    ...
    # 4. Normalize ALL features using Track A stats
    features_normalized = normalize_track_b_features(features, norm_stats)
    
    # 5. Compute scores for ALL pool candidates
    ...
    # 6. Greedy farthest-point selection from Track A passed
    selected_indices, selection_trace = greedy_farthest_point_selection(
        features=features,  # Use raw features for distance computation
        scores=scores,
    ...
    # 7. Canonicalize: sort by global_idx (GPT P0-6)
    selected_indices_canonical = np.sort(selected_indices)
    
    # 8. Build feature spec with hash
    feature_spec = TRACK_B_FEATURE_SPEC.copy()
    feature_spec['alpha'] = alpha
    feature_spec['min_distance_ratio'] = min_distance_ratio
    spec_str = json.dumps(feature_spec, sort_keys=True)
    feature_spec['spec_hash'] = hashlib.sha256(spec_str.encode()).hexdigest()[:16]
    
    # 9. Save artifacts if results_dir provided (GPT P0-5)
    artifacts_saved = {}
    if results_dir is not None:
        ...
        # track_b_candidate_features.npy
    ...
    # Print selected score stats
    selected_scores = scores[selected_indices_canonical]
    print(f"  Selected score: mean={selected_scores.mean():.4f}, range=[{selected_scores.min():.4f}, {selected_scores.max():.4f}]")
...
class Gate3TreatRunner:
    ...
    def run(self) -> Dict[str, Any]:
        ...
        if cfg.selection_mode == 'track_b':
            # Track B: Use dedicated track_b_selection function
            selected = track_b_selection(
                pool=pool,
                track_a_result=track_a_result,
                n_select=cfg.n_select,
                alpha=cfg.track_b_alpha,  # CLI parameter
                min_distance_ratio=cfg.track_b_min_dist_ratio,  # CLI parameter
                rng=self.rng_streams['select'],
                results_dir=self.results_dir,  # For artifact saving
            )
        else:
    ...
    def _save_artifacts(
        ...
        manifest = {
            ...
            # Track B SSOT (GPT P0-5: required if selection_mode='track_b')
            'track_b_config': {
                'enabled': cfg.selection_mode == 'track_b',
                'alpha': cfg.track_b_alpha,
                'min_distance_ratio': cfg.track_b_min_dist_ratio,
                'feature_spec_version': TRACK_B_FEATURE_SPEC.get('version', 'v0.1') if cfg.selection_mode == 'track_b' else None,
                'diversity_metric': TRACK_B_FEATURE_SPEC.get('diversity_metric', 'euclidean') if cfg.selection_mode == 'track_b' else None,
                'tie_break': TRACK_B_FEATURE_SPEC.get('tie_break', 'global_idx_asc') if cfg.selection_mode == 'track_b' else None,
                'canonicalized': True if cfg.selection_mode == 'track_b' else None,
...
class Gate3CompareRunner:
    ...
    def run(self) -> Dict[str, Any]:
        ...
        # P0-2 FIX: Store equivalence result for artifact
        gen_equiv_result = None
        if len(gen_manifests) >= 2:
...
def parse_args():
    ...
    parser.add_argument('--selection_mode', type=str, default='random',
                        choices=['random', 'track_a_filtered_random', 'track_b'],
                        help='Selection mode: random (baseline), track_a_filtered_random (filter+random), track_b (score+diversity)')
    ...
    parser.add_argument('--track_b_min_dist_ratio', type=float, default=0.1,
                        help='Track B: minimum distance ratio for diversity selection')
...
def main():
    ...
    config = Gate3Config(
        ...
        track_b_min_dist_ratio=args.track_b_min_dist_ratio,
        # Compare mode
