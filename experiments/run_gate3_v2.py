...
ëª©í‘œ: Learned generative augmentationì´ Gate2 ceilingì„ ëŒíŒŒí•˜ëŠ”ì§€ ê²€ì¦

ì„¤ê³„ì„œ: Gate3_Design_v1.3.md
- D1: Pipeline bring-up (GMM, Track A only, B=20)
- D2: Primary result (Track A+B, B=100, 2-seed)
- D3: Reproducibility & ablation

í•µì‹¬:
- GMM samplerë¡œ IC+params 6D ìƒì„±
- ODE ì‹œë®¬ë ˆì´ì…˜ìœ¼ë¡œ trajectory ìƒì„±
- Track A: ìƒìœ„ 10% error ì œê±° (OOD ë°©ì§€)
- Track B: Information-seeking ranking (D2ì—ì„œ í™œì„±í™”)
...
# Control Equivalence SSOT (Day3/Gate2ì™€ ë™ì¼)
CONTROL_EQUIVALENCE = {
    'library': 'gate0_min',
    'threshold': 0.05,
    'bootstrap_B': 20,
...
GATE3_CONFIG = {
    ...
    'max_pool_attempts': 15000,       # P0-2 FIX: 5000â†’15000 (acceptance ~26%)
    'pool_batch_size': 100,
    
    # Quality Control (dataset ìƒì„±ê³¼ ë™ì¼)
    'qc': {
        'max_x': 10.0,
        'max_theta': 3.1,
        'max_velocity': 30.0,
    ...
    # dx Computation (SAVGOL_CONFIGì™€ ë™ì¼)
    'savgol': {
        'window': 11,
        'polyorder': 3,
    },
    
    # Track A Selection (v1.3: ìƒìœ„ 10%ë§Œ ì œê±°)
    'track_a': {
        'reject_ratio': 0.10,
        'min_after_a': 800,  # Track Bìš© ìµœì†Œ í›„ë³´
        'error_type': 'normalized_rmse',  # dynamics target, per-target std
    },
    
    # Track B Selection (D2ì—ì„œ í™œì„±í™”)
    'track_b': {
        'enabled': False,  # D1ì—ì„œëŠ” ë¹„í™œì„±í™”
        'n_select': 200,
        'weights': {
            'residual': 0.4,
            'coverage': 0.3,
    ...
    # Physics params (fixed, train_paramsì—ì„œ m_cart, m_poleë§Œ ë³€ë™)
    'fixed_physics': {
        'L': 0.5,
        'g': 9.81,
        'b_cart': 0.1,
...
class Gate3Config:
    ...
    variant: str = "IC"  # 'IC' (u ìž¬ì‚¬ìš©) or 'ICU' (u ìƒˆë¡œ ìƒì„±)
    ...
    seed: int = 0
    day3_seed: int = None  # Day3 artifacts seed (default: same as seed)
    gmm_fit_seed: int = None  # v0.3.8: GMM fit seed (default: same as seed). For D2 cross-fit.
    pool_seed: int = None  # v0.3.6: Pool sampling seed (default: same as seed). For pool-only replication.
    note: str = "gate3"
    n_train: int = 10
    ...
    track_b_top_m_ratio: float = 5.0  # M = ratio Ã— n_select (default 5.0)
    ...
    track_b_dopt_use_teacher_intersection: bool = True  # F = fragile âˆ© teacher_active
    ...
    # v0.3.4: Top-L Pareto (GPT P1 recommendation)
    track_b_dopt_selection_variant: str = 'greedy'  # 'greedy' (original) or 'topL_pareto'
    track_b_dopt_topL_L: int = 25  # L for Top-L Pareto (default 25)
...
def create_rng_streams(base_seed: int, pool_seed: int = None) -> Dict[str, np.random.Generator]:
    """
    Create 3 independent RNG streams using SeedSequence.
    
    v0.3.6: pool_seed separation for pool-only replication
    
    P0-1 SSOT: RNG ë¶„ë¦¬ë¡œ ìž¬í˜„ì„± ë³´ìž¥
    - rng_pool: Pool generation (GMM sampling, u selection) - uses pool_seed if provided
    - rng_select: Final selection (random sampling from Track A passed)
    - rng_bootstrap: E-SINDy bootstrap resampling
    
    Args:
        base_seed: Base seed for SeedSequence (train_seed)
        pool_seed: Pool generation seed (default: same as base_seed)
    ...
    # v0.3.6: pool_seed separation
    effective_pool_seed = pool_seed if pool_seed is not None else base_seed
    
    ss = SeedSequence(base_seed)
    child_seeds = ss.spawn(3)
    
    # Pool RNG uses pool_seed (independent from base_seed if specified)
    if pool_seed is not None:
        pool_ss = SeedSequence(pool_seed)
        pool_rng = default_rng(pool_ss)
        pool_entropy = pool_ss.entropy
    else:
        pool_rng = default_rng(child_seeds[0])
        pool_entropy = ss.entropy
    ...
    return {
        ...
        '_pool_seed': effective_pool_seed,  # v0.3.6: track pool_seed
        '_pool_entropy': pool_entropy,  # v0.3.6: pool RNG entropy
    }
...
def safe_float(val) -> Optional[float]:
    """numpy scalarë¥¼ Python floatë¡œ ì•ˆì „í•˜ê²Œ ë³€í™˜"""
    if val is None:
        return None
...
def get_control_equivalence(bootstrap_B: int = 20) -> Dict:
    """Control equivalence ì„¤ì • ë°˜í™˜ (bootstrap_B ì˜¤ë²„ë¼ì´ë“œ ê°€ëŠ¥)"""
...
def load_teacher_coefficients(teacher_run_id: str, project_root: Path, config: 'Gate3Config') -> Tuple[np.ndarray, List[str], List[str]]:
    ...
    # Use day3_seed for teacher path (teacher is from baseline experiment)
    teacher_seed = config.day3_seed if config.day3_seed is not None else config.seed
    teacher_dir = (
        project_root / 'results' / config.dataset_version / 'gate1' /
        config.track / 'esindy' / f"n{config.n_train}" / f"seed{teacher_seed}" / teacher_run_id
    )
...
# ============================================================
# CartPole Dynamics (Inline - simulator import ëŒ€ì²´)
# ============================================================
...
class GMMProposalSampler:
    ...
    def fit(
        ...
        if N < 5 * self.n_components:
            ...
            if N >= 5:
                ...
                print(f"  âš ï¸ GMM: Reduced components {self.n_components} â†’ {effective_n_components} (N={N} < 5*K)")
            else:
                effective_n_components = 1
                effective_covariance = 'diag'
                print(f"  âš ï¸ GMM: Switched to K=1, diag covariance (N={N} too small)")
        ...
        print(f"  GMM fitted: {N} samples â†’ {effective_n_components} components ({effective_covariance})")
        ...
        # v0.3.6: Store fit data hash for SSOT verification
        self._fit_data_hash = compute_array_hash(data)
        
        return self
    
    def get_params_dict(self) -> Dict[str, Any]:
        """
        v0.3.6: Export GMM parameters for SSOT verification.
        
        Returns:
            Dict with GMM parameters (serializable to JSON)
        """
        if not self._fitted:
            return {}
        
        return {
            'n_components': int(self._effective_n_components),
            'covariance_type': self._effective_covariance,
            'weights': self.gmm.weights_.tolist(),
            'means': self.gmm.means_.tolist(),
            'covariances': self.gmm.covariances_.tolist(),
            'random_state': self.random_state,
            'train_mean': self._train_mean.tolist(),
            'train_std': self._train_std.tolist(),
            'train_min': self._train_min.tolist(),
            'train_max': self._train_max.tolist(),
            'fit_data_hash': self._fit_data_hash,
        }
    ...
    def sample(
        ...
        Args:
            ...
            bounds_sigma: Clip to mean Â± bounds_sigma * std
...
def compute_teacher_alignment_error(
    ...
    - dynamics_target_indices = [1, 3] â†’ x_ddot, theta_ddot
    ...
    # SSOT: dynamics targets only (v1.3 ì„¤ê³„ì„œ)
...
class PoolGenerator:
    ...
    def generate_pool(
        ...
        print(f"  âœ… Pool complete: {n_accepted}/{n_attempts} accepted ({acceptance_rate:.1%})")
...
def track_a_selection(
    ...
    v1.3: ìƒìœ„ 10%ë§Œ ì œê±° (reject_ratio=0.10)
    ...
    if pre_relax_n_pass < min_after_a and N < min_after_a:
        # Pool too small for min_after_a requirement â†’ relax to accept all
        relaxed = True
        selected_indices = np.arange(N)
        error_threshold = errors.max() + 1e-6  # Accept all
        effective_reject_ratio = 0.0
        print(f"  âš ï¸ RELAXED: pool({N}) < min_after_a({min_after_a})")
    ...
    if relaxed:
        print(f"  âš ï¸ relaxed_flag=True, effective_reject_ratio={effective_reject_ratio:.1%}")
...
def final_selection(
    ...
    It's: reject top reject_ratio error â†’ random from remaining.
    ...
    if n_available < n_select:
        print(f"  âš ï¸ Only {n_available} available, selecting all")
...
# - P0-2: Î¸ ì£¼ê¸° ë³€ìˆ˜ ì²˜ë¦¬ (sin/cos)
# - P0-3: Median/IQR ê¸°ë°˜ ì •ê·œí™” + eps
# - P0-5: ì•„í‹°íŒ©íŠ¸ ë´‰ì¸
# - P0-6: Canonicalize (global_idx ì •ë ¬)
# ============================================================
...
def compute_track_b_features(
    ...
    GPT P0-2: Î¸ëŠ” ì£¼ê¸° ë³€ìˆ˜ â†’ std(sin(Î¸)), std(cos(Î¸)) ì‚¬ìš©
...
def compute_track_b_normalization_stats(
    ...
    GPT P0-3: median/IQR ê¸°ë°˜ ì •ê·œí™”
...
def top_m_diversity_selection(
    ...
    GPT P0-2 FIX: Hard threshold â†’ Top-M gateë¡œ "ì €í’ˆì§ˆ ê°•ì œ ìˆ˜ì§‘" ë°©ì§€
    GPT P0-3 FIX: feature_vector â†’ IC/params ê³µê°„ì—ì„œ diversity
    
    Algorithm:
    1. (Optional) Score floor: score < score_floorì¸ í›„ë³´ ì œì™¸
    2. Top-M gate: score ìƒìœ„ Mê°œë§Œ diversity í›„ë³´ë¡œ (M = top_m_ratio Ã— n_select)
    3. Greedy farthest-point: IC/params ê³µê°„ì—ì„œ diversity selection
    ...
    Args:
        ...
        n_select: Number to select
        top_m_ratio: M = top_m_ratio Ã— n_select (default 5.0)
    ...
    if score_floor is not None:
        ...
        if n_above_floor < n_select:
            print(f"  âš ï¸ Score floor: only {n_above_floor} above {score_floor}, using all")
...
def track_b_selection(
    ...
    GPT P0 í”¼ë“œë°± ë°˜ì˜ (v0.2):
    - P0-2 FIX: Top-M gateë¡œ "ì €í’ˆì§ˆ ê°•ì œ ìˆ˜ì§‘" ë°©ì§€
    - P0-3 FIX: IC/params ê³µê°„ì—ì„œ diversity (feature_vector ëŒ€ì‹ )
    - Score floor ì˜µì…˜ ì¶”ê°€
    - diversity_modeë¡œ ablation ì§€ì› (score_only vs top_m_diversity)
    ...
    if relaxed_flag:
        print(f"  âš ï¸ Track A was RELAXED (all candidates passed)")
    ...
    if diversity_mode == 'score_only':
        # GPT ê¶Œìž¥ ablation: scoreë§Œìœ¼ë¡œ top-k ì„ íƒ (diversity ì—†ìŒ)
    ...
    if results_dir is not None:
        ...
        print(f"  âœ… Artifacts saved: {len(artifacts_saved)} files")
...
# ============================================================
# Track B v0.3: D-optimal Selection on Î˜_F
# ============================================================
# GPT P0 ë°˜ì˜:
# - P0-1: Î˜ ê³„ì‚° ì‹œ ì •ê·œí™” ìž…ë ¥ ì‚¬ìš© (norm_stats)
# - P0-2: targetë³„ F_t ì‚¬ìš© (dynamics targets)
# - P0-3: F_t = fragile_t âˆ© teacher_active_t (spurious ì–µì œ)
# ============================================================
...
def compute_fragile_feature_sets(
    ...
    GPT P0-2: targetë³„ F_t
    GPT P0-3: F_t = fragile_t âˆ© teacher_active_t
    
    Args:
        fragile_pairs: List of (feature_idx, target_idx) tuples
        teacher_support: (n_features, n_targets) boolean mask
        dynamics_target_indices: [1, 3] for x_ddot, theta_ddot
        use_teacher_intersection: If True, F_t = fragile âˆ© teacher_active
...
def compute_library_features_normalized(
    ...
    GPT P0-1: Teacher errorì™€ ë™ì¼í•œ ì •ê·œí™” ì ìš©
...
def compute_gram_contributions_by_target(
    ...
    G_i = Î˜_F^T @ Î˜_F  (summed over time)
    
    v0.3.2: gram_energy_mode ì¶”ê°€ (GPT í”¼ë“œë°±)
    - 'raw': ê¸°ì¡´ ë°©ì‹ (ì—ë„ˆì§€ ê·¸ëŒ€ë¡œ)
    - 'unit_trace': G_i â† G_i / (trace(G_i) + eps) (ì—ë„ˆì§€ ì¤‘ë¦½)
    
    v0.3.3: trace_power ì¶”ê°€ (GPT P1 recommendation)
    - 'trace_power': G_i â† G_i / (trace(G_i)**p + eps)
    ...
    for t, F_t in F_by_target.items():
        ...
        # Compute Gram: G_i = Î£_t Î˜_F[i,t]^T @ Î˜_F[i,t]
...
def greedy_dopt_selection(
    ...
    for t, G_t in G_by_target.items():
        n_F = G_t.shape[1]
        G_cumulative[t] = lambda_reg * np.eye(n_F)  # Î»I
    ...
    for step in range(n_select):
        ...
        # GPT P0-1 FIX: traceì— local_idxì™€ pool_idx ëª…í™•ížˆ êµ¬ë¶„
        selection_trace.append({
            'step': step,
        ...
        if step % 50 == 0 or step == n_select - 1:
            print(f"    D-opt step {step}: Î”logdet={best_delta:.4f}, cumulative={new_total_logdet:.4f}")
...
def greedy_dopt_topL_pareto_selection(
    G_by_target: Dict[int, np.ndarray],
    candidate_pool_indices: np.ndarray,
    ...
    """
    Greedy D-optimal selection with Top-L Pareto (ì‹ë³„ì„± + í’ˆì§ˆ ë™ì‹œ ìµœì í™”).
    
    v0.3.4 GPT P1 ê¶Œìž¥:
    - ë§¤ stepì—ì„œ Î”logdet ìƒìœ„ Lê°œ í›„ë³´ë§Œ ë‚¨ê¹€ (ì‹ë³„ì„± ìœ ì§€)
    - ê·¸ ì•ˆì—ì„œ teacher_error ìµœì†Œ ì„ íƒ (í’ˆì§ˆ í™•ë³´)
    - Î³ íŠœë‹ ì—†ì´ Pareto-style ì„ íƒ
    
    GPT P0 ìž ê¸ˆ:
    - P0-A: Top-L ì§‘í•©ì€ (-Î”logdet, pool_idx) lexsortë¡œ ê²°ì •ë¡ ì 
    - P0-A: ì„ íƒì€ (error, -Î”logdet, pool_idx) tie-break
    - P0-C: stepë³„ topL_candidates, selected info ì €ìž¥
    
    Args:
        G_by_target: {target_idx: (N_cand, |F_t|, |F_t|)} Gram contributions
        candidate_pool_indices: (N_cand,) pool indices
        candidate_errors: (N_cand,) teacher_error for each candidate (scalar, e.g., max over targets)
        n_select: Number to select
        L: Top-L í›„ë³´ ìˆ˜ (default 25)
        lambda_reg: Regularization for numerical stability
        
    Returns:
        selected_pool_indices: (n_select,) selected pool indices
        selection_trace: List of dicts with step-by-step selection info
        topL_artifacts: Dict with topL_candidates, topL_deltas, topL_errors for SSOT
    """
    N_cand = len(candidate_pool_indices)
    
    if N_cand <= n_select:
        return (
            candidate_pool_indices.copy(), 
            [{'note': 'all_candidates_selected', 'n_cand': N_cand}],
            {}
        )
    
    # Effective L (can't exceed remaining candidates)
    L_eff = min(L, N_cand)
    ...
    selection_trace = []
    selected_local = []  # Local indices (0..N_cand-1)
    remaining_mask = np.ones(N_cand, dtype=bool)
    
    # SSOT artifacts for Top-L (P0-C)
    topL_candidates_list = []  # (n_select, L_eff) pool indices
    topL_deltas_list = []      # (n_select, L_eff) delta_logdet values
    topL_errors_list = []      # (n_select, L_eff) teacher_error values
    
    # Greedy selection with Top-L Pareto
    for step in range(n_select):
        if not remaining_mask.any():
            break
        
        remaining_local_indices = np.where(remaining_mask)[0]
        n_remaining = len(remaining_local_indices)
        current_logdet = current_total_logdet()
        
        # 1. Compute delta for all remaining candidates
        deltas = np.full(n_remaining, -np.inf)
        
        for idx, local_i in enumerate(remaining_local_indices):
            new_logdet = 0.0
            for t, G_t in G_cumulative.items():
                G_i_t = G_by_target[t][local_i]
                G_new = G_t + G_i_t
                new_logdet += safe_logdet(G_new)
            deltas[idx] = new_logdet - current_logdet
        
        # 2. Get Top-L by Î”logdet (P0-A: lexsort for determinism)
        # Sort by: primary = -Î”logdet (desc), secondary = pool_idx (asc)
        remaining_pool_indices = candidate_pool_indices[remaining_local_indices]
        # lexsort sorts by last key first, so: (pool_idx, -deltas)
        sort_order = np.lexsort((remaining_pool_indices, -deltas))
        
        # Take top L_eff (or fewer if not enough remaining)
        L_this_step = min(L_eff, n_remaining)
        topL_order = sort_order[:L_this_step]
        
        topL_local = remaining_local_indices[topL_order]
        topL_pool_idx = candidate_pool_indices[topL_local]
        topL_delta = deltas[topL_order]
        topL_error = candidate_errors[topL_local]
        
        # Save Top-L artifacts (P0-C)
        # Pad with -1 if fewer than L_eff
        topL_pool_padded = np.full(L_eff, -1, dtype=np.int64)
        topL_delta_padded = np.full(L_eff, np.nan)
        topL_error_padded = np.full(L_eff, np.nan)
        topL_pool_padded[:L_this_step] = topL_pool_idx
        topL_delta_padded[:L_this_step] = topL_delta
        topL_error_padded[:L_this_step] = topL_error
        
        topL_candidates_list.append(topL_pool_padded)
        topL_deltas_list.append(topL_delta_padded)
        topL_errors_list.append(topL_error_padded)
        
        # 3. Within Top-L, select by minimum error (P0-A: tie-break)
        # Sort by: primary = error (asc), secondary = -Î”logdet (desc), tertiary = pool_idx (asc)
        # lexsort: (pool_idx, -delta, error) â†’ sorts by error first, then -delta, then pool_idx
        pareto_sort = np.lexsort((topL_pool_idx, -topL_delta, topL_error))
        best_in_topL_idx = pareto_sort[0]
        
        best_local_idx = topL_local[best_in_topL_idx]
        best_delta = topL_delta[best_in_topL_idx]
        best_error = topL_error[best_in_topL_idx]
        best_pool_idx = int(candidate_pool_indices[best_local_idx])
        
        # 4. Update cumulative Gram matrices
        for t, G_t in G_cumulative.items():
            G_cumulative[t] = G_t + G_by_target[t][best_local_idx]
        ...
        # Selection trace (P0-C)
        selection_trace.append({
            'step': step,
            'local_idx': int(best_local_idx),
            'pool_idx': best_pool_idx,
            'delta_logdet': float(best_delta),
            'selected_error': float(best_error),
            'cumulative_logdet': float(new_total_logdet),
            'n_remaining': int(remaining_mask.sum()),
            'L_this_step': L_this_step,
            'topL_error_min': float(topL_error.min()),
            'topL_error_max': float(topL_error.max()),
            'topL_delta_min': float(topL_delta.min()),
            'topL_delta_max': float(topL_delta.max()),
        })
        
        # Progress logging (every 50 steps)
        if step % 50 == 0 or step == n_select - 1:
            print(f"    TopL-Pareto step {step}: Î”logdet={best_delta:.4f}, error={best_error:.4f}, cumulative={new_total_logdet:.4f}")
    ...
    # SSOT artifacts (P0-C)
    topL_artifacts = {
        'topL_candidates': np.array(topL_candidates_list),  # (n_select, L_eff)
        'topL_deltas': np.array(topL_deltas_list),          # (n_select, L_eff)
        'topL_errors': np.array(topL_errors_list),          # (n_select, L_eff)
        'L': L,
        'L_eff': L_eff,
    }
    
    return selected_pool_indices, selection_trace, topL_artifacts
...
def track_b_dopt_selection(
    ...
    selection_variant: str = 'greedy',  # v0.3.4: 'greedy' or 'topL_pareto'
    topL_L: int = 25,  # v0.3.4: L for Top-L Pareto
    rng: Optional[np.random.Generator] = None,
    results_dir: Optional[Path] = None,
) -> Dict[str, Any]:
    """
    Track B Selection v0.3.4: D-optimal on fragile features (Î˜_F).
    
    GPT P0 í”¼ë“œë°± ë°˜ì˜:
    - P0-1: Î˜ ê³„ì‚° ì‹œ ì •ê·œí™” ìž…ë ¥
    - P0-2: targetë³„ F_t, ë¸”ë¡-í•© logdet
    - P0-3: F_t = fragile âˆ© teacher_active
    - Pre-gate: score top_M í›„ D-opt
    
    v0.3.1: SSOT ê°•í™” (pool_idx ì¶”ì , tie-break ê³ ì •)
    v0.3.2: ì—ë„ˆì§€-ì¤‘ë¦½ D-opt (gram_energy_mode='unit_trace')
    v0.3.3: trace_power ì¼ë°˜í™” (p=0=raw, p=1=unit_trace, p=0.7=partial)
    v0.3.4: Top-L Pareto (GPT P1 ê¶Œìž¥: ì‹ë³„ì„±+í’ˆì§ˆ ë™ì‹œ ìµœì í™”)
    ...
    Args:
        ...
        library: SINDy library instance
        n_select: Number to select
        top_m_ratio: Pre-gate M = ratio Ã— n_select
        lambda_reg: Regularization for logdet
        use_teacher_intersection: If True, F = fragile âˆ© teacher_active
        dynamics_target_indices: [1, 3] for x_ddot, theta_ddot
        pre_gate_mode: 'score' or 'none'
        alpha: Score alpha (for pre-gate)
        gram_energy_mode: 'raw', 'unit_trace', or 'trace_power'
        trace_power: power p for trace_power mode (default 1.0)
        selection_variant: 'greedy' (original) or 'topL_pareto' (v0.3.4)
        topL_L: L for Top-L Pareto (default 25)
    ...
    Returns:
        Dict with selected trajectories, artifacts, and metadata
    """
    print(f"\n[Track B Selection v0.3.4 D-opt] n_select={n_select}")
    ...
    print(f"  selection_variant={selection_variant}, topL_L={topL_L}")  # v0.3.4
    ...
    print(f"  Track A passed: {len(track_a_indices)}/{N_pool}")
    if relaxed_flag:
        print(f"  âš ï¸ Track A was RELAXED")
    ...
    # 4. Compute Î˜ for pre-gate candidates (P0-1: normalized)
    ...
    # 6. Selection (greedy or topL_pareto)
    topL_artifacts = None  # v0.3.4: for topL_pareto mode
    
    if selection_variant == 'topL_pareto':
        # v0.3.4 GPT P1 ê¶Œìž¥: Top-L Pareto (ì‹ë³„ì„± + í’ˆì§ˆ ë™ì‹œ ìµœì í™”)
        print(f"  Running Top-L Pareto selection (L={topL_L})...")
        
        # Get teacher errors for pre-gate candidates
        # pre_gate_indicesëŠ” Track A pass ì¤‘ì—ì„œ ì„ íƒëœ pool indices
        # track_a_result['errors']ëŠ” ì „ì²´ poolì— ëŒ€í•œ errors (N,)
        all_errors = track_a_result['errors']  # (N_total,) from track_a_selection
        
        # Map pre_gate_indices to errors
        # pre_gate_indicesëŠ” track_a_indices ë‚´ì˜ ê°’ë“¤ì´ë¯€ë¡œ ì§ì ‘ ì¸ë±ì‹± ê°€ëŠ¥
        candidate_errors = all_errors[pre_gate_indices]  # (N_pregate,)
        
        print(f"    Candidate errors: mean={candidate_errors.mean():.4f}, std={candidate_errors.std():.4f}")
        print(f"    Error range: [{candidate_errors.min():.4f}, {candidate_errors.max():.4f}]")
        
        # GPT P0-1 FIX: Pass pool indices directly (not local 0..M-1)
        selected_pool_indices, selection_trace, topL_artifacts = greedy_dopt_topL_pareto_selection(
        ...
        # Original greedy D-optimal selection
    ...
    # 8. Build feature spec (v0.3.4 with Top-L Pareto + SSOT enhancements)
    feature_spec = {
        'version': 'v0.3.4',  # Version bump for Top-L Pareto
        'method': 'd_optimal',
        ...
        'selection_variant': selection_variant,  # v0.3.4: 'greedy' or 'topL_pareto'
        'topL_L': topL_L if selection_variant == 'topL_pareto' else None,  # v0.3.4
        'tie_break': 'pool_idx_asc' if selection_variant == 'greedy' else 'error_delta_poolidx',  # v0.3.4
    ...
    if results_dir is not None:
        ...
        # track_b_dopt_trace.json (v0.3.4)
        trace_data = {
            'version': 'v0.3.4',
            'n_pool': N_pool,
            'n_track_a_passed': len(track_a_indices),
            ...
            'selection_variant': selection_variant,  # v0.3.4
            'topL_L': topL_L if selection_variant == 'topL_pareto' else None,  # v0.3.4
            'use_teacher_intersection': use_teacher_intersection,
            'pre_gate_mode': pre_gate_mode,
            'pre_gate_tie_break': 'score_desc_pool_idx_asc',
            'selection_tie_break': 'error_delta_poolidx' if selection_variant == 'topL_pareto' else 'pool_idx_asc',
        ...
        # GPT ê¶Œìž¥: pre_gate_indices.npy (audit ê°€ëŠ¥ì„±)
        ...
        # GPT ê¶Œìž¥: pre_gate_scores.npy (if available)
        ...
        # v0.3.2 GPT ê¶Œìž¥: selected indices order/canon ë¶„ë¦¬
        ...
        # v0.3.3 GPT ê¶Œìž¥: trace_by_target (ì—ë„ˆì§€ ë¶„ì„ìš© + quantiles + log_stats)
        ...
        # v0.3.4: Top-L Pareto artifacts (GPT P0-C)
        if topL_artifacts is not None and selection_variant == 'topL_pareto':
            # topL_candidates.npy: (n_select, L) pool indices
            topL_cand_path = results_dir / 'topL_candidates.npy'
            np.save(topL_cand_path, topL_artifacts['topL_candidates'])
            artifacts_saved['topL_candidates'] = str(topL_cand_path)
            
            # topL_deltas.npy: (n_select, L) delta_logdet values
            topL_deltas_path = results_dir / 'topL_deltas.npy'
            np.save(topL_deltas_path, topL_artifacts['topL_deltas'])
            artifacts_saved['topL_deltas'] = str(topL_deltas_path)
            
            # topL_errors.npy: (n_select, L) teacher_error values
            topL_errors_path = results_dir / 'topL_errors.npy'
            np.save(topL_errors_path, topL_artifacts['topL_errors'])
            artifacts_saved['topL_errors'] = str(topL_errors_path)
            
            # candidate_errors.npy: (n_pregate,) all candidate errors for audit
            if 'candidate_errors' not in artifacts_saved:
                all_errors = track_a_result['errors']
                candidate_errors = all_errors[pre_gate_indices]
                cand_err_path = results_dir / 'candidate_errors.npy'
                np.save(cand_err_path, candidate_errors)
                artifacts_saved['candidate_errors'] = str(cand_err_path)
    ...
    result = {
        ...
        'stats': {
            ...
            'version': 'v0.3.4',  # Top-L Pareto + SSOT
            'lambda_reg': lambda_reg,
            'gram_energy_mode': gram_energy_mode,  # v0.3.2
            'trace_power': trace_power,  # v0.3.3
            'selection_variant': selection_variant,  # v0.3.4
...
class Gate3TreatRunner:
    ...
    def __init__(self, config: Gate3Config):
        ...
        # v0.3.6: pool_seed separation for pool-only replication
        effective_pool_seed = config.pool_seed if config.pool_seed is not None else config.seed
        self.rng_streams = create_rng_streams(config.seed, pool_seed=effective_pool_seed)
        ...
        if config.pool_seed is not None and config.pool_seed != config.seed:
            print(f"  Pool seed: {config.pool_seed} (independent from train_seed={config.seed})")
    ...
    def _get_day3_dir(self) -> Path:
        ...
        cfg = self.config
        day3_seed = cfg.day3_seed if cfg.day3_seed is not None else cfg.seed
        return (
            self.project_root / 'results' / cfg.dataset_version / 'phase35' /
            cfg.track / cfg.method / f"n{cfg.n_train}" / f"seed{day3_seed}" / cfg.day3_run_id
        )
    ...
    def _load_day3_artifacts(self):
        ...
        if computed_sha != expected_sha:
            print(f"  âš ï¸ teacher_support SHA256 mismatch!")
            print(f"    Expected: {expected_sha[:16]}...")
            print(f"    Computed: {computed_sha[:16]}...")
        else:
            print(f"  âœ… teacher_support SHA256 verified: {computed_sha[:16]}...")
    ...
    def _load_dataset(self):
        ...
        print(f"  âœ… norm_stats loaded (for v2.1 alignment fix)")
    ...
    def _fit_gmm_sampler(self):
        ...
        self.gmm_sampler = GMMProposalSampler(
            ...
            random_state=self.config.gmm_fit_seed if self.config.gmm_fit_seed is not None else self.config.seed,
        )
        self.gmm_sampler.fit(train_x, train_params)
        
        return self.gmm_sampler
    
    def _save_gmm_params(self):
        """
        v0.3.6: Save GMM parameters for SSOT verification.
        v0.3.7: Separate gmm_fit_sha256 (excludes pool_seed) for true fit verification.
        
        This allows verification that generator_fit_seed produces identical GMM.
        """
        if self.gmm_sampler is None:
            return
        
        params_dict = self.gmm_sampler.get_params_dict()
        if not params_dict:
            return
        
        # v0.3.7: Compute gmm_fit_sha256 BEFORE adding pool_seed
        # This hash should be identical for same generator_fit_seed
        fit_only_keys = ['n_components', 'covariance_type', 'weights', 'means', 
                         'covariances', 'random_state', 'fit_data_hash']
        fit_only_dict = {k: params_dict[k] for k in fit_only_keys if k in params_dict}
        fit_only_json = json.dumps(fit_only_dict, sort_keys=True, default=_json_default)
        gmm_fit_sha = hashlib.sha256(fit_only_json.encode()).hexdigest()
        
        # Add metadata (including pool_seed)
        params_dict['generator_fit_seed'] = self.config.gmm_fit_seed if self.config.gmm_fit_seed is not None else self.config.seed
        params_dict['pool_seed'] = self.rng_streams.get('_pool_seed', self.config.seed)
        params_dict['n_train'] = self.config.n_train
        params_dict['gmm_fit_sha256'] = gmm_fit_sha  # v0.3.7: Store fit-only hash
        
        # Save to JSON
        gmm_params_path = self.results_dir / 'gmm_params.json'
        with open(gmm_params_path, 'w', encoding='utf-8') as f:
            json.dump(params_dict, f, indent=2, ensure_ascii=False, default=_json_default)
        
        # Compute full hash (includes pool_seed)
        params_json = json.dumps(params_dict, sort_keys=True, default=_json_default)
        gmm_params_sha = hashlib.sha256(params_json.encode()).hexdigest()
        
        print(f"  âœ… Saved: gmm_params.json")
        print(f"     gmm_fit_sha256: {gmm_fit_sha[:16]}... (fit-only, pool_seed ì œì™¸)")
        print(f"     gmm_params_sha256: {gmm_params_sha[:16]}... (full)")
        
        # Store for manifest
        self._gmm_fit_sha256 = gmm_fit_sha  # v0.3.7: fit-only hash
        self._gmm_params_sha256 = gmm_params_sha  # full hash
    ...
    def run(self) -> Dict[str, Any]:
        ...
        if cfg.pool_source:
            ...
            # v0.3.6: Save gmm_params.json for SSOT verification
            self._save_gmm_params()
        ...
        if cfg.selection_mode == 'track_b':
            ...
            selected = track_b_selection(
                ...
                top_m_ratio=cfg.track_b_top_m_ratio,        # v0.2: M = ratio Ã— n_select
            ...
            selected = track_b_dopt_selection(
                ...
                selection_variant=cfg.track_b_dopt_selection_variant,  # v0.3.4
                topL_L=cfg.track_b_dopt_topL_L,  # v0.3.4
                rng=self.rng_streams['select'],
                results_dir=self.results_dir,
            )
        else:
        ...
        train_dx = self.dataset['train_dx_savgol'][:cfg.n_train]  # §3.3 consistency
    ...
    def _load_existing_pool(self, pool_source: str) -> Tuple[Dict, Dict]:
        ...
        # v0.3.7: Load pool_source metadata from manifest.json
        pool_source_dir = pool_path.parent
        pool_source_manifest_path = pool_source_dir / 'manifest.json'
        pool_source_gmm_params_path = pool_source_dir / 'gmm_params.json'
        
        if pool_source_manifest_path.exists():
            with open(pool_source_manifest_path, 'r', encoding='utf-8') as f:
                source_manifest = json.load(f)
            
            # Extract pool_seed from source manifest
            d1_rebaseline = source_manifest.get('d1_rebaseline', {})
            pool_meta['pool_source_pool_seed'] = d1_rebaseline.get('pool_seed')
            pool_meta['pool_source_generator_fit_seed'] = d1_rebaseline.get('generator_fit_seed')
            
            # Extract gmm_fit_sha256 from source manifest
            gmm_config = source_manifest.get('gmm_config', {})
            pool_meta['pool_source_gmm_fit_sha256'] = gmm_config.get('gmm_fit_sha256')
            pool_meta['pool_source_gmm_params_sha256'] = gmm_config.get('gmm_params_sha256')
            
            print(f"  âœ… Loaded pool_source metadata from manifest.json")
            print(f"     pool_source_pool_seed: {pool_meta.get('pool_source_pool_seed')}")
            print(f"     pool_source_generator_fit_seed: {pool_meta.get('pool_source_generator_fit_seed')}")
        else:
            print(f"  âš ï¸ No manifest.json found in pool_source directory")
        
        # v0.3.7: Also try to load gmm_fit_sha256 from gmm_params.json
        if pool_source_gmm_params_path.exists() and 'pool_source_gmm_fit_sha256' not in pool_meta:
            with open(pool_source_gmm_params_path, 'r', encoding='utf-8') as f:
                gmm_params = json.load(f)
            pool_meta['pool_source_gmm_fit_sha256'] = gmm_params.get('gmm_fit_sha256')
            print(f"  âœ… Loaded gmm_fit_sha256 from gmm_params.json: {pool_meta.get('pool_source_gmm_fit_sha256', 'N/A')[:16]}...")
    ...
    def _build_gmm_config(self, pool_meta: Optional[Dict]) -> Dict:
        ...
        v0.3.6: Add gmm_params_sha256 for SSOT verification
        """
        is_pool_loaded = bool(pool_meta and pool_meta.get('pool_source'))
        ...
        if is_pool_loaded:
            ...
            # v0.3.6: Get gmm_params hash for SSOT
            # v0.3.7: Separate gmm_fit_sha256 (excludes pool_seed)
            gmm_fit_sha = getattr(self, '_gmm_fit_sha256', None)
            gmm_params_sha = getattr(self, '_gmm_params_sha256', None)
            ...
            return {
                ...
                'reg_covar': 1e-6,
                'generator_fit_seed': self.config.gmm_fit_seed if self.config.gmm_fit_seed is not None else self.config.seed,  # v0.3.8: GMM fit seed
                'gmm_fit_sha256': gmm_fit_sha,  # v0.3.7: fit-only hash (pool_seed ì œì™¸)
                'gmm_params_sha256': gmm_params_sha,  # v0.3.6: full hash
            }
    ...
    def _save_artifacts(
        ...
        print(f"  âœ… Saved: z_after.npy {z_after.shape}")
        ...
        print(f"  âœ… Saved: inc_prob_after.npy {inc_prob_after.shape}")
        ...
        print(f"  âœ… Saved: selected_trajectories.npz")
        ...
        print(f"  âœ… Saved: selected_pool_indices.npy (n={len(selected['original_indices'])})")
        ...
        print(f"  âœ… Saved: track_a_pass_mask.npy, track_a_pass_indices.npy (n_pass={track_a_pass_mask.sum()})")
        
        # 3.5 SSOT: generated_pool.npz (Pool ìž¬ì‚¬ìš©ì„ ìœ„í•œ ì „ì²´ ì €ìž¥)
        # GPT/Claude í•©ì˜: ê³µì • ë¹„êµë¥¼ ìœ„í•´ ê°™ì€ poolì—ì„œ selectionë§Œ ë°”ê¿”ì•¼ í•¨
        ...
        # - For loaded pool: pool_sha256 = SHA of ORIGINAL pool (for GENâ†”GEN equivalence)
        ...
        if is_pool_loaded:
            ...
            print(f"  âœ… Saved: generated_pool.npz (n={pool['trajectories'].shape[0]}) {pool_save_note}")
        ...
        print(f"  âœ… Saved: pool_stats.json")
        ...
        # Use day3_seed for teacher path (teacher is from baseline experiment)
        teacher_seed = cfg.day3_seed if cfg.day3_seed is not None else cfg.seed
        teacher_coeff_path = (
            self.project_root / 'results' / cfg.dataset_version / 'gate1' /
            cfg.track / 'esindy' / f"n{cfg.n_train}" / f"seed{teacher_seed}" /
        ...
        manifest = {
            ...
            'd1_rebaseline': {
                ...
                # v0.3.6: pool_seed separation for pool-only replication
                'generator_fit_seed': cfg.seed,  # GMM fitting seed (= train_seed)
                'pool_seed': self.rng_streams.get('_pool_seed', cfg.seed),  # Pool sampling seed
                'pool_entropy': self.rng_streams.get('_pool_entropy'),  # Pool RNG entropy
                # v0.3.7: pool_source metadata (from source manifest)
                'pool_source_pool_seed': pool_meta.get('pool_source_pool_seed') if pool_meta else None,
                'pool_source_generator_fit_seed': pool_meta.get('pool_source_generator_fit_seed') if pool_meta else None,
                'pool_source_gmm_fit_sha256': pool_meta.get('pool_source_gmm_fit_sha256') if pool_meta else None,
            },
        ...
        with open(self.results_dir / 'sindy_coefficients.csv', 'w', encoding='utf-8') as f:
            ...
            for i, fname in enumerate(feature_names):
                ...
                f.write(f'{fname},{row}\n')
        print(f"  âœ… Saved: sindy_coefficients.csv")
...
class Gate3CompareRunner:
    ...
    P0-1 FIX: Use fragileâˆ©dynamics (n=30) subset with paired bootstrap B=100
    P0-4: CTRL250 manifest equivalence assert
    
    Primary Endpoint (Gate3 v1.3):
    - aug_pure = z_gen - z_ctrl on fragileâˆ©dynamics pairs only
    - CI via paired bootstrap (B=100, resample the n=30 indices)
    - PASS criteria: CI_lower > 0.058 (Gate2 ceiling)
    """
    
    # SSOT: fragileâˆ©dynamics definition
    ...
    def _load_fragile_pairs(self, day3_dir: Path, z_before: np.ndarray) -> List[Tuple[int, int]]:
        """
        Load or compute fragileâˆ©dynamics pairs.
        ...
        - fragileâˆ©dynamics: fragile AND target in [1, 3]
        ...
        print("  âš ï¸ fragile_pairs.json not found, computing from Day3 artifacts...")
        ...
        if teacher_support_path.exists():
            ...
            print("  âš ï¸ No teacher_support found in Day3, checking CTRL250...")
            ctrl_dir = (
        ...
        # Compute fragileâˆ©dynamics pairs using SSOT definition
        ...
        print(f"  Computed fragileâˆ©dynamics pairs (SSOT definition):")
        print(f"    supportâˆ©dynamics: n={n_support}")
        print(f"    fragileâˆ©dynamics (support AND z<{Z0}): n={n_fragile}")
        
        return fragile_pairs
    
    def _check_gen_gen_equivalence(self, gen_manifests: Dict[str, Dict]) -> Dict:
        """
        Check GENâ†”GEN equivalence for D1 Rebaseline fairness.
    ...
    def _paired_bootstrap_ci(
        ...
        """
        Compute paired bootstrap CI for aug_pure on fragileâˆ©dynamics subset.
    ...
    def run(self) -> Dict[str, Any]:
        """Run comparison and calculate aug_pure on fragileâˆ©dynamics subset."""
        cfg = self.config
        ...
        if cfg.fragile_pairs_source:
            ...
            if fp_path.exists():
                ...
                print(f"  âœ… Loaded fragile_pairs from: {cfg.fragile_pairs_source}")
        ...
        # Option 2: Check if exists in day3_dir
            ...
            fragile_pairs_source_sha = compute_file_hash(fp_path)
            fragile_pairs_loaded = True
            print(f"  âœ… Loaded fragile_pairs from: {fp_path}")
            print(f"     SHA256: {fragile_pairs_source_sha[:16]}...")
        
        # Option 3: Compute if allowed (override)
        elif cfg.allow_fragile_compute:
            print("  âš ï¸ fragile_pairs.json not found, computing (allow_fragile_compute=True)")
        ...
        if len(fragile_pairs) == 0:
            raise ValueError("No fragileâˆ©dynamics pairs found. Cannot proceed with comparison.")
        
        print(f"  fragileâˆ©dynamics pairs: n={len(fragile_pairs)}")
        ...
        # P0-C FIX: GENâ†”GEN equivalence check (D1 Rebaseline critical)
        print("\n[GENâ†”GEN Equivalence Check]")
        gen_manifests = {}
        ...
        if len(gen_manifests) == 0:
            ...
            print(f"  â„¹ï¸ Single GEN run (trivially equivalent)")
        else:
            ...
            if gen_equiv_result['all_equivalent']:
                print(f"  âœ… GENâ†”GEN equivalence: all {len(gen_equiv_result['checked_keys'])} keys match")
            else:
                print(f"  âŒ GENâ†”GEN equivalence FAILED:")
                ...
                if not cfg.allow_fragile_compute:  # Use as a proxy for "strict mode"
                    raise ValueError("D1 Rebaseline requires GENâ†”GEN equivalence. Fix mismatches or use --allow_fragile_compute to bypass.")
        ...
        for gen_run_id in cfg.gen_run_ids:
            ...
            if not gen_dir.exists():
                print(f"  âš ï¸ GEN run not found: {gen_dir}")
                continue
            ...
            try:
                ...
                if equiv_result['is_strongly_equivalent']:
                    print(f"    âœ… Strong equivalence: all keys match")
                elif equiv_result['is_equivalent']:
                    print(f"    âœ… Critical keys match ({equiv_result['n_critical_matches']})")
                    if equiv_result['n_strong_mismatches'] > 0:
                        print(f"    âš ï¸ Strong key mismatches ({equiv_result['n_strong_mismatches']}):")
                        for m in equiv_result['strong_mismatches']:
                            print(f"       {m['key']}: GEN={m['gate3']} vs CTRL={m['ctrl250']}")
                else:
                    print(f"    âš ï¸ {equiv_result['n_critical_mismatches']} critical mismatches:")
                ...
                if gen_bootstrap_B != equiv_result['ctrl_bootstrap_B']:
                    print(f"    âš ï¸âš ï¸ CRITICAL: bootstrap_B mismatch!")
                ...
                print(f"    âŒ Equivalence check failed: {e}")
            ...
            # P0-1 FIX: Calculate aug_pure on fragileâˆ©dynamics subset ONLY
        ...
        comparison_result = {
            ...
            # P0-2 FIX: Store GENâ†”GEN equivalence result
        ...
        print(f"\n  âœ… Saved: fragile_pairs.json (n={len(fragile_pairs)})")
        
        print("\n" + "=" * 60)
        print("  Comparison Summary (fragileâˆ©dynamics subset)")
        print("=" * 60)
        for r in results:
...
def parse_args():
    ...
    parser.add_argument('--day3_seed', type=int, default=None,
                        help='Seed for Day3 artifacts path (default: same as --seed)')
    parser.add_argument('--gmm_fit_seed', type=int, default=None,
                        help='v0.3.8: Seed for GMM fit (default: same as --seed). For D2 cross-fit.')
    parser.add_argument('--pool_seed', type=int, default=None,
                        help='v0.3.6: Seed for pool sampling RNG (default: same as --seed). For pool-only replication.')
    ...
    parser.add_argument('--track_b_top_m_ratio', type=float, default=5.0,
                        help='Track B: M = ratio Ã— n_select for Top-M gate (default 5.0)')
    ...
    parser.add_argument('--track_b_dopt_use_teacher_intersection', action='store_true', default=True,
                        help='D-optimal: F = fragile âˆ© teacher_active (default True)')
    ...
    # Track B v0.3.4 Top-L Pareto options (GPT P1 recommendation)
    parser.add_argument('--track_b_dopt_selection_variant', type=str, default='greedy',
                        choices=['greedy', 'topL_pareto'],
                        help='D-optimal v0.3.4: selection variant (greedy=original, topL_pareto=ì‹ë³„ì„±+í’ˆì§ˆ Pareto)')
    parser.add_argument('--track_b_dopt_topL_L', type=int, default=25,
                        help='D-optimal v0.3.4: L for Top-L Pareto (default 25)')
...
def main():
    ...
    config = Gate3Config(
        ...
        seed=args.seed,
        day3_seed=args.day3_seed,
        gmm_fit_seed=args.gmm_fit_seed,
        ...
        track_b_dopt_selection_variant=args.track_b_dopt_selection_variant,  # v0.3.4
        track_b_dopt_topL_L=args.track_b_dopt_topL_L,  # v0.3.4
        # Compare mode
    ...
    if config.mode == 'gen_treat':
        ...
        print(f"âŒ Unknown mode: {config.mode}")
        return 1
    
    try:
        result = runner.run()
        print("\n" + "=" * 60)
        print("  âœ… Gate3 Treatment Complete")
        print("=" * 60)
