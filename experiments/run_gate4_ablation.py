...
ëª©ì : D-optimal selectionì˜ ì¸ê³¼ì  ê¸°ì—¬ ê²€ì¦ (Confound-free ì„¤ê³„)

ì„¤ê³„ ì›ì¹™:
- ë™ì¼ Pool, ë‹¤ë¥¸ Selection â†’ ìˆœìˆ˜ Selection íš¨ê³¼ ì¸¡ì •
- Pool: seed=1, pool_seed=42, pool_size=2000 (ê³ ì •)
- D-optimal: 1 run
- Random: 3 runs (selection_seed=0,1,2)

í•µì‹¬ êµ¬í˜„:
- Pool ìƒì„± 1íšŒ (ê³µìœ )
- Track A í•„í„°ë§ 1íšŒ (ê³µìœ )
- Selectionë§Œ ë‹¤ë¥´ê²Œ (D-optimal vs Random)
- E-SINDy í‰ê°€ ê°ê° ìˆ˜í–‰

ì‚°ì¶œë¬¼:
- ablation_summary.json: 4 runs ë¹„êµ
- ê° runë³„: manifest.json, metrics.json, comparison_gen.json
...
# Gate3 Runnerì—ì„œ í•µì‹¬ í•¨ìˆ˜ë“¤ import
...
class AblationConfig:
    ...
    day3_source: str = ""           # Day3 baseline (teacher_support, norm_stats ë“±)
...
def random_selection_with_seed(
    ...
    Args:
        pool: Generated pool (ë™ì¼ pool_seed)
...
class AblationRunner:
    ...
    def _load_artifacts(self):
        ...
        if teacher_support_path.exists():
            ...
            print(f"  âœ… Teacher support loaded: {self.teacher_support.shape}")
        else:
        ...
        if gate1_coef_path and gate1_coef_path.exists():
            ...
            print(f"  âœ… Teacher coefficients loaded from Gate1: {self.teacher_coefficients.shape}")
            print(f"     Source: {gate1_coef_path}")
        else:
            # Fallback: compute from teacher_support (approximate)
            print(f"  âš ï¸ Gate1 coefficients not found at: {gate1_coef_path}")
        ...
        print(f"  âœ… Norm stats computed from dataset")
        ...
        if fragile_path.exists():
            ...
            print(f"  âœ… Fragile pairs loaded: n={len(self.fragile_pairs)}")
        else:
        ...
        if z_before_path.exists():
            ...
            print(f"  âœ… z_before loaded: {self.z_before.shape}, SHA={self._z_before_sha}")
            ...
            print(f"  âœ… fragile_z_before extracted: n={len(self.fragile_z_before)}")
        else:
    ...
    def _generate_shared_pool(self):
        ...
        print(f"  âœ… Pool generated: {n_generated} trajectories")
    ...
    def _run_track_a(self):
        ...
        print(f"  âœ… Track A: {n_passed}/{n_pool} passed ({n_passed/n_pool:.1%})")
    ...
    def _evaluate_selection(
        ...
        # P2 ê°€ë“œ: ceiling_margin (GPT ê¶Œìž¥)
        if ci_lower is not None:
            metrics['ceiling_margin'] = float(ci_lower - cfg.gate2_ceiling)
        else:
            metrics['ceiling_margin'] = None
        
        # P2 ê°€ë“œ: CI seed policy (GPT ê¶Œìž¥)
        metrics['ci_seed'] = cfg.seed  # í˜„ìž¬ ëª¨ë“  runì—ì„œ ë™ì¼
        metrics['ci_seed_policy'] = 'fixed_across_runs_for_paired_comparison'
        
        # P1 ê°€ë“œ: ê³ ì • ì¡°ê±´ SHA (audit trail)
        metrics['pool_sha'] = self._pool_sha
        metrics['gmm_fit_sha'] = self._gmm_fit_sha
        metrics['seed'] = cfg.seed
        metrics['pool_seed'] = cfg.pool_seed
        metrics['pool_size'] = cfg.pool_size
        metrics['n_select'] = cfg.n_select
        metrics['runner_version'] = 'v3.1'  # ë²„ì „ íƒœê·¸
        ...
        # P1 ê°€ë“œ: fragile_z_before.npy ì €ìž¥ (GPT ê¶Œìž¥)
        np.save(results_dir / 'fragile_z_before.npy', self.fragile_z_before[:n_fragile_pairs_effective])
        
        # P1 ê°€ë“œ: effective_pairs.json ì €ìž¥ (GPT ê¶Œìž¥)
        effective_pairs_data = {
            'pairs': [],
            'n_loaded': len(self.fragile_pairs),
            'n_effective': n_fragile_pairs_effective,
            'oob_pairs': [],
            'feature_names': self.feature_names,
            'target_names': self.target_names,
        }
        for feature_idx, target_idx in self.fragile_pairs:
            if feature_idx < z_after.shape[0] and target_idx < z_after.shape[1]:
                effective_pairs_data['pairs'].append([feature_idx, target_idx])
            else:
                effective_pairs_data['oob_pairs'].append([feature_idx, target_idx])
        ...
        print(f"  ceiling_margin: {metrics['ceiling_margin']:.4f}" if metrics['ceiling_margin'] is not None else "  ceiling_margin: N/A")
        print(f"  z_after_median: {metrics['z_after_median']:.3f}, promotion_rate: {metrics['promotion_rate']:.1%}")
        print(f"  âœ… Saved: {results_dir} (+ effective_pairs.json, fragile_z_before.npy)")
    ...
    def _save_run_artifacts(
        ...
        manifest = {
            ...
            'evaluation': {
                ...
                'ci_alpha': cfg.ci_alpha,
                'ci_seed': cfg.seed,
                'ci_seed_policy': 'fixed_across_runs_for_paired_comparison',
                'gate2_ceiling': cfg.gate2_ceiling,
            ...
            'artifacts': [
                ...
                'fragile_z_before.npy',      # P1 ê°€ë“œ ì¶”ê°€
                'effective_pairs.json',       # P1 ê°€ë“œ ì¶”ê°€
    ...
    def _generate_summary(self, all_results: Dict[str, AblationRunResult]) -> Dict[str, Any]:
        ...
        if dopt_result.status == 'completed' and dopt_metrics:
            runs_list.append({
                ...
                'ceiling_margin': dopt_metrics.get('ceiling_margin'),  # P2 ê°€ë“œ
                # Secondary metrics
        ...
        for r in random_results:
            if r.status == 'completed' and r.metrics:
                runs_list.append({
                    ...
                    'ceiling_margin': r.metrics.get('ceiling_margin'),  # P2 ê°€ë“œ
                    # Secondary metrics
        ...
        summary = {
            ...
            'comparison': {
                'd_optimal': {
                    ...
                    'ceiling_margin': dopt_metrics.get('ceiling_margin') if dopt_metrics else None,  # P2 ê°€ë“œ
                    # Secondary
        ...
        print(f"  Random:    {summary['comparison']['random']['median_aug_pure_mean']:.3f} Â± {summary['comparison']['random']['median_aug_pure_std']:.3f}" if summary['comparison']['random']['median_aug_pure_mean'] else "  Random: N/A")
        print(f"\n[Pass Level]")
        ...
        print(f"  Random:    {summary['comparison']['random']['z_after_median_mean']:.3f} Â± {summary['comparison']['random']['z_after_median_std']:.3f}" if summary['comparison']['random']['z_after_median_mean'] else "  Random: N/A")
        print(f"\n[Verdict]")
...
def main():
    ...
    if summary.get('status') == 'failed':
        print(f"âŒ Failed: {summary.get('error')}")
        sys.exit(1)
