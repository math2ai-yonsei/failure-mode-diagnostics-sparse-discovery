...
데이터 출처: Results Tables v1.8 (POST-PATCH)
출력: figures/ 디렉토리 (PNG + PDF 듀얼)

생성 Figure:
    fig2_gate3_pass_level    — Cart-Pole augmentation pass-level summary (18 runs)
    fig3_cp_dopt_random      — Cart-Pole D-opt vs Random (10 seeds)
    fig4_aek_history         — AEK conditioning and augmentation history (noise-free)
    fig5_aek_coverage        — AEK Coverage Gate diagnosis (std_ratio)
    fig6_applicability       — Applicability Condition (kappa + std(cos))
    fig7_silverbox_random    — Silverbox Random score_aligned distribution (D-opt point only)
    fig8_4system_verdict     — 4-System D-opt Selection Verdict (small-multiples, main text)
...
# Cart-Pole D-opt vs Random (POST-PATCH SavGol)
...
# Silverbox Random score_aligned distribution
...
# AEK kappa progression (historical Standard → Reparam-1 → Reparam-2 stages)
AEK_KAPPA = {
    'Standard\nlibrary':      4.7e9,
    'Reparam-1':              4.5e4,
    'Reparam-2\n(exploratory)': 473,
...
COVERAGE = {
    ...
    'θ̇_w²':         {'baseline': 0.15, 'dither': 1.29, 'threshold': 0.50},
}

# Applicability Condition (Table 8)
AC_DATA = {
    'AEK':       {'std_cos': 0.0034, 'delta_log_kappa': +5.02, 'pass': True},
    'CP':        {'std_cos': 0.037, 'delta_log_kappa': -0.07, 'pass': False},
}

# 5-System summary — used only by Fig A2 (Appendix) for Lynx-Hare data.
# Main-text Fig 8 uses its own 4-system local data.
# DOPT_SCORES   = [+0.294, None,   +0.827, +2.513, +0.410]  # archived
# RANDOM_SCORES = [+0.066, -0.280, +1.525, +2.503, +0.478]  # archived
# SYSTEM_COLORS and FAILURE_MODES — archived; Fig 8 uses local 4-system data
...
# =============================================================================
# Cart-Pole pass-level distribution
# =============================================================================
def plot_fig2_gate3_pass_level():
    """Cart-Pole: 18-run summary with pass-level breakdown."""
    ...
    ax.set_title('Cart-Pole Augmentation Pass-Level Summary (18 runs)')
    ax.set_xlim(0, 20)
    ...
    fig.tight_layout()
    save_figure(fig, 'fig2_gate3_pass_level')


# =============================================================================
# Cart-Pole D-optimal vs Random
# =============================================================================
def plot_fig3_cp_dopt_random():
    """Cart-Pole D-opt vs Random 10 seeds strip plot."""
    ...
    ax.set_title('Cart-Pole — D-optimal vs Random Selection')
    ax.set_xlim(0.5, 2.5)
    ax.legend(fontsize=FONT_SIZES['legend'])
...
# =============================================================================
# AEK conditioning and augmentation history
# =============================================================================
def plot_fig4_aek_history():
    ...
    # Right: AEK augmentation verdict (categorical — no unsupported medians)
    # SSOT: Std Random = 10/10 NULL (no median reported), RP1 baseline = NULL:8/SOFT:2,
    #        RP1 dither = 2/10 SOFT median=-0.280
    milestones = [
        ('Std\nbaseline',   '10/10 NULL',           COLORS['null']),
        ('RP1\nbaseline',   'NULL:8\nSOFT:2',      COLORS['null']),
        ('RP1\ndither',     '2/10 SOFT\nmed=-0.280', COLORS['soft']),
    ]
    x_pos = np.arange(len(milestones))
    for xi, (lab, verdict, col) in enumerate(milestones):
        ax2.text(xi, 0.5, verdict, ha='center', va='center',
                 fontsize=FONT_SIZES['annot'] + 1, fontweight='bold', color=col,
                 bbox=dict(boxstyle='round,pad=0.4', facecolor='#F8F8F8',
                           edgecolor=col, alpha=0.9))
    ax2.set_xticks(x_pos)
    ax2.set_xticklabels([m[0] for m in milestones], fontsize=FONT_SIZES['tick'])
    ax2.set_title('(b) AEK Augmentation Verdict')
    ...
    ax2.annotate('', xy=(2.0, 0.3), xytext=(0.0, 0.3),
                 arrowprops=dict(arrowstyle='->', color='#999', lw=1.5))
    ax2.text(1.0, 0.22, 'diagnostic intervention progression',
             ha='center', va='top', fontsize=7, color='#888', style='italic')

    fig.suptitle('AEK — Conditioning Improvement and Augmentation History',
...
def plot_fig5_coverage():
    ...
    ax.set_title('AEK Coverage Gate Diagnosis')
    ax.legend(fontsize=FONT_SIZES['legend'])

    # Threshold annotation
    ax.text(1.45, COVERAGE['θ̇_w²']['threshold'] + 0.03,
...
def plot_fig6_applicability():
    """Applicability Condition — std(cos(theta)) vs kappa improvement."""
    ...
    # Manual label offsets to avoid dot/threshold overlap
    label_offsets = {'AEK': (0.003, -0.4), 'CP': (-0.004, 0.45)}
    for s, x, y in zip(systems, std_vals, delta_log):
        dx, dy = label_offsets.get(s, (0.015, 0))
        ax.text(x + dx, y + dy, s, fontsize=FONT_SIZES['label'],
    ...
    ax.set_title('Applicability Condition — Reparameterization Effect')
...
# =============================================================================
# 4-System D-optimal verdict figure (main text)
# =============================================================================
def plot_fig8_verdict():
    ...
    for i, ax in enumerate(axes):
        ...
        ax.text(0.12, random_4[i], f'{random_4[i]:+.3f}', va='center', ha='left',
    ...
    fig.suptitle('D-optimal Selection Verdict — 4 Main-Text Systems\n'
    ...
    save_figure(fig, 'fig8_4system_verdict')


# =============================================================================
# Silverbox Random score_aligned distribution
# =============================================================================
def plot_fig7_silverbox():
    ...
    # differing z-score scales. Silverbox headline = CI consistently positive, not ceiling comparison.

    ax.set_xticks([1, 2])
    ax.set_xticklabels(['Random\n(10 seeds)', 'D-optimal\n(point est. only)'],
                       fontsize=FONT_SIZES['label'])
    ax.set_ylabel('score_aligned (positive = improvement)')
    ax.set_title('Silverbox — GMM Augmentation Results\n'
                 '(Real Engineering Data, κ=3.814)')
    ax.legend(fontsize=FONT_SIZES['legend'] - 1, borderpad=0.3, labelspacing=0.3)

    # CI consistently positive annotation
    ax.text(0.98, 0.05,
            '10/10 SOFT_PASS\nCI consistently positive\n[+2.419, +2.553]',
            transform=ax.transAxes, ha='right', va='bottom',
    ...
    ax.set_ylim(-0.5, 4.0)
    fig.tight_layout()
    save_figure(fig, 'fig7_silverbox_random')
...
def plot_figA2_lynxhare():
    ...
    ax.set_xticklabels(['Random\n(10 seeds)', 'D-optimal'], fontsize=FONT_SIZES['label'])
    ax.set_ylabel('score_aligned (positive = improvement)')
    ax.set_title('Lynx-Hare — Appendix Positive Evidence\n'
