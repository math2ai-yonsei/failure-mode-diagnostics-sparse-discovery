...
def greedy_dopt_selection(
    ...
    Args:
        ...
        candidate_indices: (N_cand,) pool indices of candidates
        n_select: Number to select
    ...
    for step in range(n_select):
        ...
        deltas = np.full(len(remaining_indices), -np.inf)
        
        for idx, local_i in enumerate(remaining_indices):
        ...
        best_local_idx = remaining_indices[best_idx]
        best_delta = deltas[best_idx]
        
        # Tie-break: if multiple have same delta, pick smallest global_idx
        tie_mask = np.abs(deltas - best_delta) < 1e-12
        if tie_mask.sum() > 1:
            tie_remaining = remaining_indices[tie_mask]
            tie_global = candidate_indices[tie_remaining]
            best_local_idx = tie_remaining[np.argmin(tie_global)]
            best_delta = deltas[tie_mask][np.argmin(tie_global)]
...
def track_b_dopt_selection(
    ...
    """
    Track B Selection v0.3: D-optimal on fragile features (Θ_F).
    ...
    Returns:
        Dict with selected trajectories, artifacts, and metadata
    """
    print(f"\n[Track B Selection v0.3 D-opt] n_select={n_select}")
    ...
    if pre_gate_mode == 'score':
        ...
        sorted_by_score = np.argsort(-track_a_scores)[:M]
        pre_gate_indices = track_a_indices[sorted_by_score]
        pre_gate_scores = track_a_scores[sorted_by_score]
    ...
    # 5. Compute Gram contributions per target
    print(f"  Computing Gram contributions...")
    G_by_target = compute_gram_contributions_by_target(Theta, F_by_target)
    
    for t, G_t in G_by_target.items():
        target_name = ['x', 'x_ddot', 'theta', 'theta_ddot'][t]
        print(f"  G_{target_name}: shape={G_t.shape}")
    
    # 6. Greedy D-optimal selection
    print(f"  Running greedy D-opt selection...")
    # Note: candidate_indices for D-opt are 0..len(pre_gate_indices)-1
    # We need to map back to pool indices
    local_candidate_indices = np.arange(len(pre_gate_indices))
    
    selected_local, selection_trace = greedy_dopt_selection(
    ...
    selected_pool_indices = pre_gate_indices[selected_local]
    n_selected = len(selected_pool_indices)
    print(f"  Selected: {n_selected} trajectories")
    
    # 7. Canonicalize
    selected_indices_canonical = np.sort(selected_pool_indices)
    
    # 8. Build feature spec
    feature_spec = {
        'version': 'v0.3',
        'method': 'd_optimal',
