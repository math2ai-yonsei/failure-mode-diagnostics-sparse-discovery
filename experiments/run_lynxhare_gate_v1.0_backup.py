...
def generate_pool(
    ...
    while len(pool_x) < cfg.pool_size and attempts < cfg.max_pool_attempts:
        ...
        # Simulate LV trajectory
        x_traj = simulate_lv_trajectory(
            H0, L0, lv_params,
            T_steps=cfg.T_steps,
            dt=cfg.dt,
            max_state=cfg.qc_max_state,
        ...
        dx_traj = compute_lv_derivatives(x_traj, lv_params)

        # QC: reject if any state < min or > max
        if np.any(x_traj < cfg.qc_min_state) or np.any(x_traj > cfg.qc_max_state):
            n_rejected += 1
            continue
        if not np.all(np.isfinite(dx_traj)):
            n_rejected += 1
            continue
