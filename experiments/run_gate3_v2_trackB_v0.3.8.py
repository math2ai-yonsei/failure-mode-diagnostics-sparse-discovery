...
def greedy_dopt_topL_pareto_selection(
    ...
    """
    Greedy D-optimal selection with Top-L Pareto (식별성 + 품질 동시 최적화).
    
    v0.3.4 GPT P1 권장:
    - 매 step에서 Δlogdet 상위 L개 후보만 남김 (식별성 유지)
    - 그 안에서 teacher_error 최소 선택 (품질 확보)
    - γ 튜닝 없이 Pareto-style 선택
    
    GPT P0 잠금:
    - P0-A: Top-L 집합은 (-Δlogdet, pool_idx) lexsort로 결정론적
    - P0-A: 선택은 (error, -Δlogdet, pool_idx) tie-break
    - P0-C: step별 topL_candidates, selected info 저장
    ...
    for step in range(n_select):
        ...
        # 2. Get Top-L by Δlogdet (P0-A: lexsort for determinism)
        # Sort by: primary = -Δlogdet (desc), secondary = pool_idx (asc)
        ...
        # Sort by: primary = error (asc), secondary = -Δlogdet (desc), tertiary = pool_idx (asc)
        # lexsort: (pool_idx, -delta, error) → sorts by error first, then -delta, then pool_idx
        ...
        # Progress logging (every 50 steps)
        if step % 50 == 0 or step == n_select - 1:
            print(f"    TopL-Pareto step {step}: Δlogdet={best_delta:.4f}, error={best_error:.4f}, cumulative={new_total_logdet:.4f}")
...
def track_b_dopt_selection(
    ...
    """
    Track B Selection v0.3.4: D-optimal on fragile features (Θ_F).
    ...
    v0.3.4: Top-L Pareto (GPT P1 권장: 식별성+품질 동시 최적화)
    ...
    if selection_variant == 'topL_pareto':
        # v0.3.4 GPT P1 권장: Top-L Pareto (식별성 + 품질 동시 최적화)
        print(f"  Running Top-L Pareto selection (L={topL_L})...")
        
        # Get teacher errors for pre-gate candidates
        # pre_gate_indices는 Track A pass 중에서 선택된 pool indices
        # track_a_result['errors']는 전체 pool에 대한 errors (N,)
        all_errors = track_a_result['errors']  # (N_total,) from track_a_selection
        
        # Map pre_gate_indices to errors
        # pre_gate_indices는 track_a_indices 내의 값들이므로 직접 인덱싱 가능
...
class Gate3TreatRunner:
    ...
    def _save_gmm_params(self):
        ...
        print(f"  ✅ Saved: gmm_params.json")
        print(f"     gmm_fit_sha256: {gmm_fit_sha[:16]}... (fit-only, pool_seed 제외)")
    ...
    def _load_existing_pool(self, pool_source: str) -> Tuple[Dict, Dict]:
        ...
        if pool_source_manifest_path.exists():
            ...
            print(f"  ✅ Loaded pool_source metadata from manifest.json")
            ...
            print(f"  ⚠️ No manifest.json found in pool_source directory")
        ...
        if pool_source_gmm_params_path.exists() and 'pool_source_gmm_fit_sha256' not in pool_meta:
            ...
            print(f"  ✅ Loaded gmm_fit_sha256 from gmm_params.json: {pool_meta.get('pool_source_gmm_fit_sha256', 'N/A')[:16]}...")
    ...
    def _build_gmm_config(self, pool_meta: Optional[Dict]) -> Dict:
        ...
        if is_pool_loaded:
            ...
            return {
                ...
                'gmm_fit_sha256': gmm_fit_sha,  # v0.3.7: fit-only hash (pool_seed 제외)
...
def parse_args():
    ...
    parser.add_argument('--track_b_dopt_selection_variant', type=str, default='greedy',
                        ...
                        help='D-optimal v0.3.4: selection variant (greedy=original, topL_pareto=식별성+품질 Pareto)')
