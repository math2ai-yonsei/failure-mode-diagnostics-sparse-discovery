"""
AEK-4c Dither: Random Selection + Dither PD (Reparam-1, Coverage-Improved)
...
Date: 2026-03-04 (Dither PD patch from run_aek4c_random.py)
Runner version: v1.0
"""
...
class AEK4cConfig:
    ...
    pd_gain_margin: float = 1.5        # Kp = 1.5 * Mgh (relaxed for coverage)
    pd_Kd_factor: float = 0.15         # Kd = 0.15 * Kp
    pd_noise_std: float = 0.003        # higher noise for coverage
    ...
    # Dither reference (coverage improvement)
    dither_amplitude: float = 0.015   # rad (~0.86 deg)
    dither_freq: float = 0.5          # Hz
...
def generate_pool(
    ...
    """
    Generate augmentation pool via AEK simulator with SavGol dx.

    Steps:
        1. Estimate PD gains from training data
        2. Sample ICs + I_w_C from GMM
        3. Simulate with PD stabilizing controller + noise
        4. Compute SavGol dx from clean teacher trajectory (§3.3)
    ...
    while n_accepted < cfg.pool_size and n_attempted < cfg.max_pool_attempts:
        ...
        for i in range(batch):
            ...
            # PD stabilizing controller + dither reference + exploration noise
            noise_seq = rng.normal(0, noise_std, size=T)

            # Dither reference: sinusoidal phi target with random phase
            if cfg.dither_amplitude > 0:
                t_grid = np.linspace(0, cfg.T_duration, T)
                dither_phase = rng.uniform(0, 2 * np.pi)
                dither_ref = cfg.dither_amplitude * np.sin(
                    2 * np.pi * cfg.dither_freq * t_grid + dither_phase)
            else:
                dither_ref = np.zeros(T)

            def _make_pd_ctrl(kp, kd, noise, dref, dt_val, T_val):
                def ctrl(t, x):
                    k = min(int(t / dt_val), T_val - 1)
                    phi_error = x[0] - dref[k]
                    tau = kp * phi_error + kd * x[1] + noise[k]
                    return tau
                return ctrl

            controller = _make_pd_ctrl(Kp, Kd, noise_seq, dither_ref, dt, T)
...
def save_run(
    ...
    manifest = {
        ...
        'runner': 'experiments/run_aek4c_dither_random.py',
        'runner_version': RUNNER_VERSION,
        'pool_sha': pool_sha,
...
def load_baseline(cfg: AEK4cConfig) -> Dict[str, Any]:
    """
    Load Reparam-1 baseline artifacts.
    ...
    if cfg.baseline_dir:
        ...
        # Find run with 'rp1' in name (most recent first)
        candidates = sorted(base.iterdir(), reverse=True)
        bdir = None
        for c in candidates:
            if c.is_dir() and 'rp1' in c.name:
                bdir = c
                break
...
class AEK4cRunner:
    ...
    def run(self) -> Dict[str, Any]:
        ...
        print("=" * 70)
        print("AEK-4c Dither: Random Selection + Dither PD (RP1, Coverage-Improved)")
        ...
        pool_sha = hashlib.sha256(pool['trajectories'].tobytes()).hexdigest()[:16]
        ...
        results_base = paths.ROOT / 'results' / cfg.dataset_version / 'gate4c-2' / 'random_dither'
        all_results = {}
        ...
        for sel_seed in cfg.random_seeds:
            ...
            try:
                ...
                # Save
                run_id = paths.generate_run_id(f"aek4c_dither_s{sel_seed}")
        ...
        if completed:
            ...
            # Save summary JSON
            summary = {
                'system': 'aek', 'gate': 'gate4c-2', 'method': 'random_dither',
                'reparam': cfg.reparam,
            ...
            summary_path = results_base / 'aek4c_dither_random_summary.json'
...
def parse_args():
    p = argparse.ArgumentParser(description='AEK-4c Dither Random Augmentation (Coverage-Improved)')
