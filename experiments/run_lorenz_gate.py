...
Date: 2026-03-05
Runner version: v1.1 (pool dx → SavGol for §3.3 consistency; previously analytic)
"""
...
RUNNER_VERSION = 'v1.1'  # v1.1: pool dx → SavGol (논문 §3.3 일관성)
        ...
        for i in range(len(ics_batch)):
            ...
            try:
                ...
                if sim.is_bounded(x_arr, max_norm=cfg.qc_max_state_norm):
                    # v1.1: SavGol derivatives from clean teacher trajectory
                    # 논문 §3.3 일관성: "processed with the same SavGol estimation"
                    # No noise added to pool (consistent with CP pool design).
                    dx_savgol = np.zeros_like(x_arr)
                    for s in range(3):
                    ...
                    pool_x.append(x_arr)          # (T, 3) clean state
                    pool_dx.append(dx_savgol)     # (T, 3) SavGol derivative
                    pool_rho.append(rho)
                    pool_ics.append(ic)
                    n_accepted += 1
            except RuntimeError:
...
def _write_context_packet(
    ...
    lines += [
        ...
        f"",
        f"## SSOT Notes",
        f"- score_aligned sign: determined by runtime failure_mode",
