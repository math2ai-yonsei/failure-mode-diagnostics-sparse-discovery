...
class AblationRunner:
    ...
    def _evaluate_selection(
        ...
        fragile_z_after = []
        for feature_idx, target_idx in self.fragile_pairs:  # FIX: 순서 수정
        ...
        metrics = {
            'method': method_name,
            'n_fragile_pairs_loaded': len(self.fragile_pairs),
            'n_fragile_pairs_effective': n_fragile_pairs_effective,
        ...
        # Add E-SINDy support summary
        support_mask = eval_result['support_mask']
        stable_mask = eval_result['stable_core_mask']
        fragile_mask = eval_result['fragile_pool_mask']
        metrics['support_size'] = int(support_mask.sum())
        metrics['stable_size'] = int(stable_mask.sum())
        metrics['fragile_size'] = int(fragile_mask.sum())
        
        # Save z_after
        np.save(results_dir / 'z_after.npy', z_after)
        np.save(results_dir / 'fragile_z_after.npy', fragile_z_after)
        
        print(f"  Support: {metrics['support_size']}, Stable: {metrics['stable_size']}, Fragile: {metrics['fragile_size']}")
    ...
    def _compute_verdict(
        ...
        # Simple heuristic comparison (NOT a statistical test)
        # Note: With n=3 random samples, formal significance testing is not meaningful
        advantage_outside_1std = abs(dopt_advantage) > random_std if random_std > 0 else dopt_advantage != 0
        
        if dopt_z > random_mean and advantage_outside_1std:
            conclusion = 'DOPT_ADVANTAGE_OBSERVED'
        elif dopt_z < random_mean and advantage_outside_1std:
        ...
        return {
            ...
            'note': 'Heuristic comparison only. With n=3 random samples, formal significance testing is not applicable.',
        }
