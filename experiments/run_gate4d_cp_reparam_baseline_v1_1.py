...
def save_baseline_artifacts(
    ...
    # P0: data-independent theta_sha (feature-name based, not data bytes)
    feat_names_for_sha = get_cp_feature_names(reparam)
    theta_sha_lib = hashlib.sha256(
        ('\n'.join(feat_names_for_sha) + reparam).encode()
    ).hexdigest()[:16]
    ...
    manifest = {
        ...
        'reparam': reparam,
        'library_id': 'RP1' if reparam == 'reparam1' else 'Standard',
        'library_version': 'Reparam-1' if reparam == 'reparam1' else 'Standard',
        'reparam_indices': [5, 14, 16, 18, 20] if reparam == 'reparam1' else [],
        # P0: data-independent library signature
        'theta_sha': theta_sha_lib,
        'theta_sha_definition': 'sha256(feature_names_joined + reparam_str)[:16]',
        # P0: key experiment parameters for reproducibility
        'dataset_version': dataset_version,
        'n_train': n_train,
        'n_bootstrap': 100,   # from ESINDy call
        'threshold': 0.05,
        'z_fragile_threshold': float(fragile_info['z_threshold']),
        'baseline_seed': baseline_seed,
        'created_at': datetime.now().isoformat(),
